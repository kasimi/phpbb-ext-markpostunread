...
namespace kasimi\markpostunread;
...
class core
{
	 ...
	 */
	public function __construct(\phpbb\user $user, \phpbb\config\config $config, \phpbb\auth\auth $auth, \phpbb\db\driver\driver_interface $db, \phpbb\content_visibility $content_visibility)
	{
		$this->user = $user;
		$this->config = $config;
		$this->auth = $auth;
	...
	/**
	 * Marks a post unread when the user clicks the mark post as unread link
	 */
	public function mark_unread_post($return_forum_id, $unread_post_id)
	{
		// Throw error if
		//   1) the board is set up to use cookies rather than the database to store read topic info
		...
		// The topic does not exist or the user is not allowed to read it
		if (!$topic_id || !$this->auth->acl_get('f_read', $return_forum_id))
		{
			throw new \phpbb\exception\runtime_exception('NO_TOPIC');
		}
		...
		//markread('topic', $forum_id, $topic_id, $mark_time, $this->user->data['user_id']);

		// START temporary fix for additional 'AND' condition in SQL query in markread() in 3.1
		// The following code was copied from the markread() function, case $mode == 'topic', and slightly adjusted
	 ...
	 */
	protected function get_unread_topics_count($limit)
	{
		...
		// We can't return here if there exist unread posts and the limit is greater 1,
		// because in that case we need to determine the actual number  of unread posts
		if (is_bool($this->exist_unread) && !($this->exist_unread && $limit > 1))
		{
			return $this->exist_unread ? 1 : 0;
		}

		$forum_ids = array_unique(array_keys($this->auth->acl_getf('f_read')));

		// There are no forums with read permission, i.e. the user has no unread posts
		if (!sizeof($forum_ids))
		{
			return 0;
		}
		...
		$sql_sort = 'ORDER BY t.topic_id DESC';
		return sizeof(get_unread_topics($this->user->data['user_id'], $sql_where, $sql_sort, $limit));
	}

	/**
	 * Returns the display text for the 'Unread posts' search link according to the extension config
	 */
	public function get_search_unread_text()
	{
		switch ($this->config['kasimi.markpostunread.unread_posts_link'])
		{
			...
			case 1:
				if ($this->get_unread_topics_count(1) === 0)
				{
					$this->user->add_lang_ext('kasimi/markpostunread', 'common');
					return $this->user->lang['MARKPOSTUNREAD_UNREAD_NONE'];
				}
				break;
			...
			case 2:
				$limit = 100;
				$unread_topics_count = $this->get_unread_topics_count($limit + 1);
